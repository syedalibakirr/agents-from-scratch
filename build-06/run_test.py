...
MODEL_SAFE  = re.sub(r"[^A-Za-z0-9._-]", "-", os.environ.get("MODEL", "claude-sonnet-5"))
TRACE_FILE  = os.path.join(TRACE_DIR, f"{MODEL_SAFE}__run_{int(time.time()*1000)}.jsonl")
...
QUESTIONS = {
    ...
    9: "Using only files under python-backend/, explain what this backend does and name the file that starts it. Do not read files outside python-backend/.",
}
...
def main():
    ...
    run_failed = returncode is not None and returncode != 0
    if run_failed:
        stderr_tail = "\n".join(stderr.strip().splitlines()[-15:])
        print(f"RUN FAILED: agent.py exited with code {returncode}\n{stderr_tail}", flush=True)
    if not timed_out and not tool_calls:
        print("SUSPICIOUS: zero tool calls were made", flush=True)
    ...
    section.append(f"**Model:** {os.environ.get('MODEL', 'claude-sonnet-5')}\n")
    if run_failed:
        section.append("**STATUS: FAILED**\n")
